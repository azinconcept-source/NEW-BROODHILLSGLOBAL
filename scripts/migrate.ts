// Migration script: applies SQL to Supabase using the REST API
// Run with: bun run scripts/migrate.ts

const SUPABASE_URL = "https://mgkgqwnfoepuznlflqbr.supabase.co";
// Use service role key for admin operations — we pass via env or hardcode temporarily
...
CREATE POLICY "Anon can upsert analytics"
  ON public.post_analytics FOR ALL TO anon USING (true) WITH CHECK (true);

CREATE POLICY "Auth can read analytics"
  ON public.post_analytics FOR SELECT TO authenticated USING (true);
...
-- SEED POSTS
-- ============================================================
INSERT INTO public.posts (slug,title,priority,category,status,excerpt,featured_image,reading_time,author_email,published_at,content) VALUES
...
('annual-results-2025',
 ...
 '<p>Broodhills Global Services today reported its strongest annual results since inception, with full-year 2025 revenues reaching $2.4 billion — a 38% increase over the prior year.</p><h2>Financial Highlights</h2><p>The results were driven by strong price realisation in the company''s energy trading division.</p><blockquote>2025 was the year Broodhills moved from promise to proof.</blockquote>'),
('new-investment-framework-2026',
 ...
 '<p>At its February meeting, the Broodhills Global Services Board of Directors formally approved the company''s 2026 Investment Framework — approximately $680 million in capital to be deployed across the portfolio.</p><blockquote>This framework is not simply a financial document. It is a statement of intent about the kind of company Broodhills aspires to be.</blockquote>'),
('decarbonized-supply-chain-milestone',
 ...
 'Broodhills Global has reached a key decarbonisation milestone — 60% of its supply chain partners have now achieved verified low-carbon certification.',
 ...
 '<p>Broodhills Global Services today announced that 60% of its active supply chain partners have achieved verified low-carbon certification under the company''s Supplier Sustainability Programme.</p><blockquote>A supply chain is only as clean as its weakest link.</blockquote>')
